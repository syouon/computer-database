...
<ul class="pagination">
	<c:if test="${page-1 != 0}">
		<li><mlib:link range="${currentRange}" page="${page-1}"
				target="dashboard" orderBy="${orderBy}" desc="${desc}"
				search="${search}">
		...
		<c:when test="${pageNumber <= 2}">
			<c:forEach var="i" begin="1" end="${pageNumber+1}">
				<li><mlib:link target="dashboard" page="${i}"
						range="${currentRange}" orderBy="${orderBy}" desc="${desc}"
						search="${search}">${i}</mlib:link></li>
			</c:forEach>
		</c:when>
		<c:when test="${page <= 2}">
	...
	<c:if test="${page <= pageNumber}">
		<li><mlib:link range="${currentRange}" page="${page+1}"
				target="dashboard" ariaValue="Next" orderBy="${orderBy}"
				desc="${desc}" search="${search}">
